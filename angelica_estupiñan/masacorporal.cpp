...
{
	float imc,peso,altura;//declaramos los variables de imc,peso y altura con float
	cout<<"Ingrese el peso (kg): ";cin>>peso;//se muestra  mensaje en pantalla al usuario que ingrese y lo guardamos
	cout<<"Ingrese la altura (metros): ";cin>>altura;//pedimos al usuario que ingrese su altura y lo guardamos
	imc=peso/(altura*altura)//escribimos el imc general para calcular
			;
		
	if(imc<18.5){//utilizamos si es verdadero la condicion 
		cout<<"Usted tiene bajo peso"<<endl;//se muestra al usuario el resultado por pantalla 
	}
	if(imc>=18.5 && imc<24.9){//utilizamos si la condicion se cumple
		cout<<"Usted tiene peso normal"<<endl;//se muestra el mensaje al usuario su resultado por pantalla
	}
	...
	if(imc>=27 && imc<29.9){//utilizamos con la condicion verdadero si es  mayor  
		cout<<"Usted tiene sobrepeso grado II"<<endl;//se muestra el resultado al usuario por pantalla
	}

	if(imc>=30 && imc<34.9){//con la condicion se cumple si es mayor o menor
		cout<<"Usted tiene obesidad tipo I"<<endl;// se muestra el resultado en pantalla
	}

	if(imc>=35 && imc<39.9){//utilizamos si es verdadero la condicion de imc  
		cout<<"Usted tiene obesidad tipo II"<<endl;//se muestra el resultado del valor ingresado por el usuario
	}

	if(imc>=40 && imc<49.9){//imc es mayor o menor que 
